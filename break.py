...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", required=True, help="Name of scan output ")
    parser.add_argument("--file", required=True,help="Bed File")
    parser.add_argument("--indexpeak", required=True,help="Peak index to break")
    parser.add_argument("--breaklevel", required=True,help="Level of peak to break")
    parser.add_argument("--lastfirst", required=True,help="The first level of the last analysis ")
    parser.add_argument("--levels", default=10, help="Maximum decomposition level")
    ...
    parser.add_argument("--first", default=1, help="(First level to Maximum level) for signal scanning after 1M values levels after 10 is recommended " )
...
with open(f'{args.input}_peak_Ids.pkl', "rb") as file:
    resultsAllInput = pickle.load(file)


###Original file
filename=args.file
dataI = np.genfromtxt(filename, delimiter=',')

######Option 1 (Select a signal from the decomposition table)
# Selecte the level and signal index

levelToBreak=int(args.breaklevel)-int(args.lastfirst)
# print(levelToBreak)
signaIndexToBreak=int(args.indexpeak)

startSubS=resultsAllInput[levelToBreak][signaIndexToBreak][0]
endSubS=resultsAllInput[levelToBreak][signaIndexToBreak][1]

# ############ Option 2 (Input your own signal)
# # start=
# # end=

####Subseting
sigIndex = (dataI[:, 0] >= startSubS) & (dataI[:, 0] <= endSubS)
dataToBreak = dataI[sigIndex]
...
with open(f'{startSubS}_{endSubS}_decomposition_table.pkl', "wb") as file_recV:
...
with open(f'{startSubS}_{endSubS}_peak_Ids.pkl', "wb") as file:
...
plt.legend(loc='best')
plt.savefig(f'{startSubS}_{endSubS}_diagnostic.png')
